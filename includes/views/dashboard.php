...
<div class="wrap pv-wrap">
	<div class="pv-brand-header">
		...
		<div class="pv-brand-header__text">
			<h1 class="pv-title"><?php esc_html_e( 'Visitor Sentinel', 'visitor-sentinel' ); ?></h1>
			<p class="pv-subtitle"><?php esc_html_e( 'Traffic monitoring and automatic protection.', 'visitor-sentinel' ); ?></p>
		</div>
	</div>

	<div class="pv-cards">
		<div class="pv-card pv-card--live">
			<span class="pv-card__label"><span class="pv-live-dot" aria-hidden="true"></span><?php esc_html_e( 'Online right now', 'visitor-sentinel' ); ?></span>
			<span class="pv-card__value" id="pv-online-now-value"><?php echo esc_html( number_format_i18n( $online ) ); ?></span>
		</div>
		<div class="pv-card"><span class="pv-card__label"><?php esc_html_e( 'Unique visitors today', 'visitor-sentinel' ); ?></span><span class="pv-card__value"><?php echo esc_html( number_format_i18n( $today ) ); ?></span></div>
		<div class="pv-card"><span class="pv-card__label"><?php esc_html_e( 'Visits (last 7 days)', 'visitor-sentinel' ); ?></span><span class="pv-card__value"><?php echo esc_html( number_format_i18n( $week_visits ) ); ?></span></div>
		<div class="pv-card pv-card--danger"><span class="pv-card__label"><?php esc_html_e( 'Currently blocked IPs', 'visitor-sentinel' ); ?></span><span class="pv-card__value"><?php echo esc_html( number_format_i18n( $active_bans ) ); ?></span></div>
		<div class="pv-card"><span class="pv-card__label"><?php esc_html_e( 'Total recorded blocks', 'visitor-sentinel' ); ?></span><span class="pv-card__value"><?php echo esc_html( number_format_i18n( $total_bans ) ); ?></span></div>
	</div>
	...
	<div class="pv-panel">
		...
		<?php $chart_max = ! empty( $daily_visits ) ? max( max( $daily_visits ), 1 ) : 1; ?>
		<div class="pv-chart">
					...
					<?php foreach ( $top_pages as $page ) : ?>
						<li>
							<span class="pv-list__label">
								<?php if ( 0 === strpos( $page->request_uri, '/wp-admin' ) ) : ?><span class="pv-badge pv-badge--admin"><?php esc_html_e( 'Admin', 'visitor-sentinel' ); ?></span><?php endif; ?>
			...
			<?php else : ?>
				<ul class="pv-list">
					<?php foreach ( $top_referrers as $referrer ) : ?>
						<li><span class="pv-list__label"><?php echo esc_html( $referrer->referer ); ?></span><span class="pv-list__count"><?php echo esc_html( number_format_i18n( $referrer->total ) ); ?></span></li>
					<?php endforeach; ?>
				</ul>
			<?php endif; ?>
		</div>
	...
	<div class="pv-grid-2">
		<div class="pv-panel">
			...
			<?php if ( empty( $event_breakdown ) ) : ?>
				<p class="pv-empty"><?php esc_html_e( 'No suspicious activity detected.', 'visitor-sentinel' ); ?></p>
			<?php else : ?>
		...
		<div class="pv-panel">
			...
			<?php
			$device_labels = array( 'desktop' => __( 'Desktop', 'visitor-sentinel' ), 'mobile' => __( 'Mobile', 'visitor-sentinel' ), 'tablet' => __( 'Tablet', 'visitor-sentinel' ), 'bot' => __( 'Bots / automated', 'visitor-sentinel' ) );
	...
	<div class="pv-panel">
		<h2 class="pv-panel-title"><span class="pv-panel-title__icon"><?php VISISE_Icons::render( 'globe', 17 ); ?></span><?php esc_html_e( 'Geographic threat distribution (30 days)', 'visitor-sentinel' ); ?></h2>
		<?php if ( empty( $geo_stats ) ) : ?>
			<p class="pv-empty"><?php esc_html_e( 'Enable Geo Lookup in Settings to see country data.', 'visitor-sentinel' ); ?></p>
		<?php else : ?>
			<ul class="pv-list">
				<?php foreach ( $geo_stats as $g ) : ?>
					<li>
						<span class="pv-list__label"><span class="pv-flag"><?php echo esc_html( strtoupper( $g->country_code ) ); ?></span> <?php echo esc_html( $g->country_code ); ?></span>
						<span class="pv-list__count"><?php echo esc_html( number_format_i18n( $g->total ) ); ?></span>
					</li>
				<?php endforeach; ?>
			</ul>
		<?php endif; ?>
	...
	<div class="pv-panel">
		...
		<?php if ( empty( $recent_events ) ) : ?>
			<p class="pv-empty"><?php esc_html_e( 'No recent suspicious activity.', 'visitor-sentinel' ); ?></p>
		<?php else : ?>
			<div class="pv-table-wrap">
				<table class="widefat pv-table">
					<thead><tr><th><?php esc_html_e( 'Date', 'visitor-sentinel' ); ?></th><th><?php esc_html_e( 'IP', 'visitor-sentinel' ); ?></th><th><?php esc_html_e( 'Event', 'visitor-sentinel' ); ?></th><th><?php esc_html_e( 'Details', 'visitor-sentinel' ); ?></th><th><?php esc_html_e( 'Score', 'visitor-sentinel' ); ?></th></tr></thead>
					<tbody>
						<?php foreach ( $recent_events as $event ) : ?>
							<tr>
								<td><?php echo esc_html( mysql2date( 'd.m.Y H:i:s', $event->created_at ) ); ?></td>
								<td><a href="<?php echo esc_url( add_query_arg( array( 'page' => 'visise-bans', 'ip' => $event->ip ), admin_url( 'admin.php' ) ) ); ?>"><?php echo esc_html( $event->ip ); ?></a></td>
